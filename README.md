...
You paste a task, pick a complexity (1–5), and three AI stages turn it into cleaned code & 
a pool of judge models scores each stage and picks the best.  
...
Each stage has its own on/off toggle. If a model hits a paywall/quota, untick it a disabled stage shows a 
**"skipped"** marker and forwards the previous stage's output (stage 1 off = your raw input passes straight to Groq).  
  
## Judge pool (picks the best output)  
After the three stages run, a pool of **judge models** (`JUDGE_MODELS`) each score every stage's output 
from 0–100 against your original request. Any judge that errors or returns non-numeric output is 
**discarded** and replaced by the next spare from `JUDGE_FALLBACK_MODELS`. The surviving numeric scores 
are **averaged** per stage, and the stage with the **highest average** is reported as the best.  
...
Each provider rotates through a per-tier list of model slugs; if one returns junk or errors, it rotates to the next. 
The panel shows **which model was used** and a **per-stage confidence %** (the averaged judge score).  
  
## Safety features  

- **Safetywall:** each stage re-checks its own output and retries on junk, rotating through the tier's
  fallback slugs (`MAX_SAFETYWALL_TRIES`, default 8, env-overridable). Set it at least as large as your longest tier
  list so every fallback slug is reachable.  
- **Rate-limit throttle:** a delay before every outbound call (`RATE_LIMIT_DELAY`, default 6s) to avoid free-tier burst 429s.  
- **Reasoning flag:** every configured slug (generation tiers + judges) is called with `reasoning: {enabled: true}`.  
...
The installer detects the SSD, mounts it at /mnt/dizerdata, prompts for your API keys in order (Gemini → OpenRouter → Groq), 
installs the dizercore service, and prints the URL using the Pi's detected IP. If it applies the USB UAS quirk, it asks you 
to reboot and re-run once.
...
## Usage
    ...
    - Untick any stage you want to skip (e.g. Gemini when you've hit its daily cap), or untick Judge pool to skip scoring.
    - Click Run. Watch each stage populate with its output, model used, and the averaged judge confidence.
    - Clear chat empties the prompt box; the red × on a history item deletes it.
    - Tip on complexity: if a 3 gives poor code, re-run the same task at 4 or 5 to route it to heavier models.
